...
dc_ip  = node['windows_ad']['dc_ip'] ||
         search(:node, 'recipe:test_windows_ad\\:\\:setup_dc')[0]['ipaddress']

puts "DEBUG:: IP => #{dc_ip}"

WindowsHelper.nic_interface_indexes.each do |index|
  powershell_script "set DNS servers for NIC #{index}" do
    code "Set-DNSClientServerAddress -interfaceIndex #{index}
...
execute "net localgroup \"Administrators\" \"#{domain_admin}\" /add" do
  not_if "net localgroup Administrators |
          grep -i #{admin} |
          grep -i #{short_domain}"
end
